...
const ev_FoundNumber = new CustomEvent("FoundNumber"); 
...
window.addEventListener("load", async ()=>{
	const thumnail_tile = document.querySelector("#thumnail_tile");
	const image_dialog = document.querySelector("#image_dialog");
	var image_canvas = document.querySelector("#image_canvas");
	CurrentImageCanvas = image_canvas;
	...
	async function main() {
		...
		Signal.addEventListener("FoundNumber", ()=>{
			const num = NumberOfImages;
			console.log(`Number of Image: ${num}`);
			FindingNumer = false;
			no.value = num;

			no.value = NumberOfImages;
			PageNo = NumberOfImages;
		...
		Signal.addEventListener("EndRec", async (e)=>{
			if(e.detail.type == "jpeg"){
				...
				function imgprocess(url){
					...
					CurrentImageCanvas.replaceWith(canvas2);
					CurrentImageCanvas = canvas2;
					image_canvas.replaceWith(canvas2);
					image_canvas.className = "full_image_canvas";
					loading.close();
...
1
			...
			function thumY2PNM(y){
				...
				thumcanvas.id = PageNo;
				thumcanvas.className = "canvas_thumnail"
				thumcanvas.addEventListener("click", ()=>{
					loading.showModal();
					const num = thumcanvas.id;
					no.value = num;
					PageNo = num;
					getFirstImg();
				...
				thumnail_tile.appendChild(thumcanvas);
		...
		baudRate.addEventListener("click", chgbaudrate);
